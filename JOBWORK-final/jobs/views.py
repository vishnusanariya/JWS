...
def jobUpload(request):
    if request.method == 'POST':
        ...
        if Profile.objects.get(regis=user)!= None:
            ...
            job.save()
            messages.success(request,'job requirement uploaded successfully please continue to add another')
            return render(request,'jobs/requirement-upload.html',{'job':jobs(),'username':username})
        else:
            messages.warning(request,'Please complete Profile details first')
